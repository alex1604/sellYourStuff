...
const defaultimg =
  "https://firebasestorage.googleapis.com/v0/b/sellyourstuff-b27b2.appspot.com/o/productImages%2Fplaceholder.png?alt=media&token=230fbf95-36cf-4508-a1bb-985511142a86";
...
class App extends Component {
  ...
  updateProductInfo = () => {
    ...
    fetch(urlFetch, { method: "PUT", body: body })
      .then((res) => {})
        ...
        this.setState(
          {
            max: data.max,
            min: data.min,
            priceRange: { myMin: data.min, myMax: data.max },
            loaded: true,
          },
          () => {}
        );
  ...
  removeFromCart = (itemToDelete) => {
    ...
    let newCart = this.state.shoppingCart.filter(
  ...
  filterMeBabyOhYeahFilterMePlease = (category, priceRange) => {
    let q, min, max;
    if (
      (priceRange.myMin === this.state.min &&
        priceRange.myMax === this.state.max) ||
      typeof priceRange.myMin != "number" ||
      typeof priceRange.myMax != "number"
    ) {
      if (category === "all") {
        fetch("http://localhost:3000/api/products/", {
          method: "GET",
        ...
        q = "";
        fetch("http://localhost:3000/api/filter/" + category + q, {
          method: "GET",
          headers: {
            "Access-Control-Allow-Origin": "*",
          },
  ...
  addPrice = (price) => {
    ...
    this.setState({ priceRange: price }, callback);
  };
  ...
  render() {
    ...
    const sayWelcome = !this.state.isLoggedIn ? null : (
      <div
        style={{
          textAlign: "center",
          marginBottom: "20px",
    ...
    const landingPage =
      this.state.currentTab === "landing" && !this.state.isLoggedIn ? (
    ...
    if (this.state.currentTab === "shop") {
      currentApp = (
        <ProductList
          ...
          minRange={this.state.min}
          maxRange={this.state.max}
    ...
    const createFormPage = !this.state.user ? null : (
      <Create tabClick={this.tabClick} userProps={this.state.user} />
    );
        ...
        {this.state.loaded && this.state.loaded2 ? (
          <div>
            ...
            <main className="mainView">
              ...
              <div
                id="productsPage"
                className={this.state.currentTab === "shop" ? "show" : "hide"}
              >
                {sayWelcome}
                {currentApp}
              </div>
              ...
              <div
                id="createPage"
                className={
                  this.state.currentTab === "editForm" &&
                  this.state.isLoggedIn === true
                    ? "show"
                    : "hide"
                }
